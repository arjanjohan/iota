...
const TYPO_ID: &IdentStr = ident_str!("ID");
const TYPO_OPTION: &IdentStr = ident_str!("Option");
const TYPO_STRING: &IdentStr = ident_str!("String");
const TYPO_UID: &IdentStr = ident_str!("UID");
    ...
    fn try_from(value: A::MoveValue) -> Result<Self, Error> {
        ...
        Ok(match value {
            ...
            V::Struct(s) => {
                ...
                if is_type(&type_, &STD, MOD_OPTION, TYPO_OPTION) {
                    // 0x1::option::Option
                    Self::Option(match extract_option(&type_, fields)? {
                        Some(value) => Some(Box::new(MoveData::try_from(value)?)),
                        None => None,
                    })
                } else if is_type(&type_, &STD, MOD_ASCII, TYPO_STRING)
                    || is_type(&type_, &STD, MOD_STRING, TYPO_STRING)
                {
                    // 0x1::ascii::String, 0x1::string::String
                    Self::String(extract_string(&type_, fields)?)
                } else if is_type(&type_, &IOTA, MOD_OBJECT, TYPO_UID) {
                    // 0x2::object::UID
                    Self::Uid(extract_uid(&type_, fields)?.into())
                } else if is_type(&type_, &IOTA, MOD_OBJECT, TYPO_ID) {
                    // 0x2::object::ID
...
fn extract_uid(
    ...
    let A::MoveStruct { type_, fields } = s;
    if !is_type(&type_, &IOTA, MOD_OBJECT, TYPO_ID) {
        return Err(Error::Internal(
