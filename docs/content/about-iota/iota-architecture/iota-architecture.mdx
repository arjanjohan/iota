...
## Sequencing
After consensus provides the set of transactions to be included in the ledger, they must be sequenced to determine their exact order. IOTA uses a unique sequencing algorithm aimed at maximizing throughput by making optimal use of concurrent processing capabilities.
